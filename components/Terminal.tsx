...
export const Terminal: React.FC<TerminalProps> = ({ activeTab, onTabChange, onAction }) => {
  ...
  const executeCommand = async (command: string): Promise<string> => {
    ...
    // ---------------------------------------------------------
    // 실제 API 연동 시 아래 주석을 해제하고 fetch/axios 등을 사용하세요.
    /*
    try {
      const response = await fetch('/api/terminal/execute', { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ command, cwd: '/app' }) 
      });
      const data = await response.json();
      return data.output;
    } catch (e) {
      return "Error: Failed to connect to terminal server.";
    }
    */
